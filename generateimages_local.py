...
HIGHRES_FIX_DENOISING = 0.35 #int(os.environ.get('HIGHRES_FIX'))
...
# MODEL_ID = 'paularedygg'#os.getenv('MODEL_ID')
# MODEL_KEY = 'portrait of paularedygg'#os.environ.get('MODEL_KEY')
# MODEL_CLASS = 'person'#os.environ.get('MODEL_CLASS')
...
# HIGHRES_FIX = 1 #int(os.environ.get('HIGHRES_FIX'))
...
# for char_prompt in charPrompts:
#     result = search_lexica('man ' + char_prompt)
#     if result.get('images') != None:
#         parsed = result['images']
#         for promptjson in parsed:
#             print(promptjson['prompt'])
...
for prompt in splittedPrompts:
    ...
    while ctr < BATCH_SAMPLES:
        ...
        automatic1111_run_txt2img({
                    ...
                    'negative_prompt': 'real life photo, sex, kissing, skin, naked, blowjob, titties, sexy, explicit, sensual, breast, group, harem, love making, cock, penis',
                ...
                # aws s3 sync /workspace/stable-diffusion-webui/outputs s3://aimodels-cyian/models/biancakes/outputs  --exclude "*.ckpt"
        # subprocess.run([
        #     "python3", 
        #     "/workspace/imagegen/cleanup.py"
        # ])
        # subprocess.run([
        #     "mv", 
        ...
        # ])
        subprocess.run([
            "/workspace/imagegen/face_detect_purge.sh"
        ])
        subprocess.run([
            "aws", 
            "s3", 
